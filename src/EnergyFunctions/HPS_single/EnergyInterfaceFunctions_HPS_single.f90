...
!****************************************************************************************
      module E_Interface_HPS_single
      use CoordinateTypes
!=============================================================================
      contains
!=============================================================================      
! Calculates the total energy of the initial configuration for the HPS_single force field
! (hydrophobic-polar model for biomolecules) in a grand canonical ensemble nucleation
! simulation. Checks if the configuration forms a single cluster using one of three
! criteria: energy-based (interaction energy below threshold), distance-based (first
! atom distance below threshold), or minimum distance-based (any atom pair distance
! below threshold). Sets rejectMove to .true. if the cluster criterion fails. Uses
! stored pair distances if useDistStore=.true. for faster calculations.
subroutine Detailed_ECalc_HPS_single(E_T, rejectMove)
  use InterEnergy_HPS_single, only: Detailed_ECalc_Inter
  use IntraEnergy_HPS_single, only: Detailed_ECalc_IntraNonBonded, Detailed_ECalc_GasIntra
  ...
  use E_Interface_HPS_single_Diststore, only: Detailed_ECalc_HPS_single_DStore
  ...
  if (useDistStore) then
    call Detailed_ECalc_HPS_single_DStore(E_T, rejectMove)
    return
  endif
  ...
  ! Calculate intermolecular energies for HPS_single force field
  ...
  ! Add bending and torsional energy contributions (currently zero for HPS_single)
  E_Bend_T = 0.0_dp
  E_T = E_T + E_Bend_T
  E_Torsion_T = 0.0_dp
  E_T = E_T + E_Torsion_T
end subroutine Detailed_ECalc_HPS_single
!=============================================================================      
...
! using the HPS_single force field. Computes intermolecular (E_Inter) and intramolecular (E_Intra)
...
! useDistStore=.true. (delegates to Shift_ECalc_HPS_single_DStore) and .false. (uses PairList).
! Populates NeighborDetailsNew for minDistCriteria when provided (e.g., LLTranslation).
subroutine Shift_ECalc_HPS_single(E_Inter, E_Intra, disp, PairList, dETable, useIntra, rejMove, &
                                  useInter, NeighborDetailsNew)
  use VarPrecision, only: dp
  use SimParameters, only: NTotal, beta, softCutOff, distCriteria, minDistCriteria
  ...
  use InterEnergy_HPS_single, only: Shift_ECalc_Inter
  use DistanceCriteria, only: Shift_DistanceCriteria
  use EnergyCriteria, only: Shift_EnergyCriteria
  use IntraEnergy_HPS_single, only: Shift_ECalc_IntraNonBonded
  ...
  use E_Interface_HPS_single_Diststore, only: Shift_ECalc_HPS_single_DStore
  ...
  if (useDistStore) then
    call Shift_ECalc_HPS_single_DStore(E_Inter, E_Intra, disp, PairList, dETable, useIntra, rejMove, &
                                       useInter, NeighborDetailsNew)
    return
  endif
  ...
  if (.not. any(useIntra) .and. abs(E_Intra) > 1.0E-6_dp) then
    write(35, *) "Error: Non-zero E_Intra=", E_Intra, " in Shift_ECalc_HPS_single with useIntra=.false."
    stop "Shift_ECalc_HPS_single: Invalid intramolecular energy"
  endif
end subroutine Shift_ECalc_HPS_single
!=============================================================================      
! Computes the intra- and intermolecular energy contributions for inserting a new molecule into the
! cluster using the HPS_single force field in a Grand Canonical Monte Carlo nucleation simulation, as
...
! Delegates to SwapIn_ECalc_HPS_single_DStore if useDistStore=.true. Supports the minimum-distance
! cluster criterion (minDistCriteria) and is extensible to LJ_Q, Mpipi, HPS_piecewise, and HPS_cation_pi.
subroutine SwapIn_ECalc_HPS_single(E_Inter, E_Intra, PairList, dETable, rejMove, useInter, NeighborDetailsNew)
  use VarPrecision, only: dp
  use SimParameters, only: beta, softCutOff
  use Coords, only: newMol
  use CBMC_Variables, only: regrowType
  use InterEnergy_HPS_single, only: NewMol_ECalc_Inter
  use IntraEnergy_HPS_single, only: NewMol_ECalc_IntraNonBonded
  use BondStretchFunctions, only: NewMol_ECalc_BondStretch
  use E_Interface_HPS_single_Diststore, only: SwapIn_ECalc_HPS_single_DStore
  use EnergyTables, only: E_Inter_Diff, E_NBond_Diff, E_Strch_Diff, E_Bend_Diff, E_Tors_Diff
  use PairStorage, only: useDistStore
  implicit none
  ...
  if (useDistStore) then
    call SwapIn_ECalc_HPS_single_DStore(E_Inter, E_Intra, PairList, dETable, rejMove, &
                                        useInter, NeighborDetailsNew)
    return
  end if
  ...
  ! HPS_single force field (U_HPS = 4 * eps * lambda * [ (sigma/r)**12 - 2 * (sigma/r)**6 ] +
  ! Debye-Hückel electrostatics). Updates PairList and dETable for energy and cluster criteria.
  ! If NeighborDetailsNew is present, updates neighbor relationships for minDistCriteria; otherwise,
  ! uses default neighbor handling. Rejects the move if overlaps occur or energy exceeds soft cutoff.
  if (interSwitch) then
    if (present(NeighborDetailsNew)) then
  ...
  ! new molecule if it is not rigid (regrowType /= 0). Uses HPS_single non-bonded interactions.
...
end subroutine SwapIn_ECalc_HPS_single
!=============================================================================
! Calculates the intermolecular and intramolecular energy of a molecule being removed in a Grand Canonical
! Monte Carlo swap-out move using the HPS_single force field with useDistStore=.false. Computes E_Inter
! (Hydropathy Scale Lennard-Jones scaled by lambda and Debye-Hückel electrostatics) and E_Intra (non-bonded,
! bond stretch, bending, torsional, and improper terms) for the specified molecule, updating dETable for
! cluster criteria. Optionally skips intermolecular interactions if useInter is present and .false.
! Used in AVBMC or similar moves.
subroutine SwapOut_ECalc_HPS_single(E_Inter, E_Intra, nType, nMol, dETable, useInter)
  use VarPrecision, only: dp
  use PairStorage, only: useDistStore
  use CBMC_Variables, only: regrowType
  use EnergyTables, only: E_Inter_Diff, E_NBond_Diff, E_Strch_Diff, E_Bend_Diff, E_Tors_Diff
  use InterEnergy_HPS_single, only: Mol_ECalc_Inter
  use IntraEnergy_HPS_single, only: Mol_ECalc_IntraNonBonded
  use BondStretchFunctions, only: Mol_ECalc_BondStretch
  use E_Interface_HPS_single_Diststore, only: SwapOut_ECalc_HPS_single_DStore
  implicit none
  ...
  if (useDistStore) then
    call SwapOut_ECalc_HPS_single_DStore(E_Inter, E_Intra, nType, nMol, dETable, useInter)
    return
  end if
  ...
  ! Overview: Computes HPS Lennard-Jones and Debye-Hückel electrostatic energies, negated for removal.
  if (interSwitch) then
...
end subroutine SwapOut_ECalc_HPS_single
!=============================================================================
