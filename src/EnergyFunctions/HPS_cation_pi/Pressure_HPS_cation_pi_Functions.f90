...
!*********************************************************************************************************************
      module Pressure_HPS_cation_pi
      use VarPrecision

      contains
!======================================================================================      
      subroutine Detailed_PCalc_HPS_cation_pi(P_T)
!      use ParallelVar
      ...
      real(dp) :: eps,sig_sq,q,lambda,rcutNBsq,epsLJ
      logical :: q_Nzero
             ...
             do iAtom = 1,nAtoms(iType)
               ...
               do jAtom = 1,nAtoms(jType)        
                 ...
                 if(r_sq .lt. rcutNBsq) then
                   ...
                   if (epsLJ .gt. 1E-5_dp) HPS = HPS + 48.0_dp * epsLJ * x * (x - 0.5_dp)
                   P_HPS = P_HPS + HPS
                 endif  
                 Ele = 0E0_dp
                 if (q_Nzero) then
...
!======================================================================================      
      subroutine Shift_PCalc_HPS_cation_pi(P_Trial, disp)
...
!      integer, pointer :: oldIndx 
      ...
      real(dp) :: eps,sig_sq,q,rcutNBsq,lambda,epsLJ
      logical :: q_Nzero
...
!======================================================================================      
      pure subroutine SwapOut_PCalc_HPS_cation_pi(iType, iMol, P_Trial)
      use ForceField, only: nAtoms, atomArray
      use ForceFieldPara_HPS_cation_pi, only: eps_tab, epsLJ_tab, q_Nonzero, q_tab, sigsq_tab, lambda_tab, cutoffNBsq_tab, rcutElecsq
      use Coords, only: Displacement,  molArray
      use SimParameters, only: nMolTypes, NPART, kapa
...
!======================================================================================      
      subroutine SwapIn_PCalc_HPS_cation_pi(P_Trial)
      use ForceField, only: nAtoms, atomArray
      use ForceFieldPara_HPS_cation_pi, only: eps_tab, epsLJ_tab, q_Nonzero, q_tab, sigsq_tab, lambda_tab, cutoffNBsq_tab, rcutElecsq
