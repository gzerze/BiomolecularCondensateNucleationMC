...
!****************************************************************************************
      module E_Interface_Mpipi_Diststore
      use CoordinateTypes
!=============================================================================
      contains
!=============================================================================
! Computes total system energy for the Mpipi force field in a grand canonical ensemble
...
subroutine Detailed_ECalc_Mpipi_DStore(E_T, rejMove)
  use InterEnergy_Mpipi_DistStore, only: Detailed_ECalc_Inter
  ...
  E_Torsion_T = 0.0_dp
  E_T = E_T + E_Torsion_T
end subroutine Detailed_ECalc_Mpipi_DStore
!=============================================================================      
! Calculates the energy change for a single molecule displacement in the Mpipi force field when
...
subroutine Shift_ECalc_Mpipi_DStore(E_Inter, E_Intra, disp, PairList, dETable, useIntra, rejMove, &
  ...
  use InterEnergy_Mpipi_DistStore, only: Shift_ECalc_Inter
  ...
  if (.not. any(useIntra) .and. abs(E_Intra) > 1.0E-6_dp) then
    write(35, *) "Error: Non-zero E_Intra=", E_Intra, " in Shift_ECalc_Mpipi_DStore with useIntra=.false."
    stop "Shift_ECalc_Mpipi_DStore: Invalid intramolecular energy"
  endif
end subroutine Shift_ECalc_Mpipi_DStore
!=============================================================================      
! Calculates the intermolecular and intramolecular energies for a newly inserted molecule in a Grand
! Canonical Monte Carlo nucleation simulation using the Mpipi force field with useDistStore=.true.
...
subroutine SwapIn_ECalc_Mpipi_DStore(E_Inter, E_Intra, PairList, dETable, rejMove, &
                                     useInter, NeighborDetailsNew)
  use InterEnergy_Mpipi_DistStore, only: NewMol_ECalc_Inter
...
end subroutine SwapIn_ECalc_Mpipi_DStore
!=============================================================================
! Calculates the intermolecular and intramolecular energy of a molecule being removed in a Grand Canonical
! Monte Carlo swap-out move using the Mpipi force field with useDistStore=.true. Computes E_Inter
! (Wang-Frenkel and Debye-Hückel electrostatics using stored distances) and E_Intra (non-bonded, bond
! stretch, bending, torsional, and improper terms) for the specified molecule, updating dETable for
...
subroutine SwapOut_ECalc_Mpipi_DStore(E_Inter, E_Intra, nType, nMol, dETable, useInter)
  use VarPrecision
  use Coords
  use CBMC_Variables, only: regrowType
  use EnergyTables, only: E_Inter_Diff, E_NBond_Diff, E_Strch_Diff, E_Bend_Diff, E_Tors_Diff
  use InterEnergy_Mpipi_DistStore, only: Mol_ECalc_Inter
  ...
  ! Overview: Computes Wang-Frenkel and Debye-Hückel electrostatic energies using stored distances, negated for removal.
  if (interSwitch) then
...
end subroutine SwapOut_ECalc_Mpipi_DStore
!=============================================================================
